...
const searchData = document.querySelector("#app-search");

const searchFunction = () => {
  let input = searchData.value.trim().toLowerCase();
  let tempData = data.filter((item) => {
    return item.spacecraft.trim().toLowerCase().includes(input);
  });
  resultList.innerHTML = "";
  tempData.forEach((item) => {
    resultList.innerHTML += `<li>${item.spacecraft}</li>`;
  });
};

searchData.addEventListener("keyup", searchFunction);
