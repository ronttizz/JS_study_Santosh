...
const sumGrades = (grades, callback) => {
  ...
  setTimeout(() => {
    ...
    if (callback) {
      callback(sum, grades); // call the success callback with the sum
    }
  }, 1000);
};
...
const calculateSum = (grades) => {
  sumGrades(grades, (sumOfGradesFromSumGrades, grades) => {
    console.log(`From grades ${grades} the sum is ${sumOfGradesFromSumGrades}`);
  });
};
